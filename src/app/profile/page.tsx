...
import { MapPin, Plus, X, Edit2, Trash2, Stethoscope, Heart, BookOpen, ShoppingCart, Package, IndianRupee } from 'lucide-react';

type ActiveTab = 'dashboard' | 'account' | 'security' | 'my-details' | 'my-pets' | 'saved-vets' | 'my-orders';
...
export default function Profile() {
  ...
  const tabs: { id: ActiveTab; label: string }[] = [
    ...
    ...((isPetOwner || isVet) ? [{ id: 'my-orders' as ActiveTab, label: 'My Orders' }] : []),
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      {/* ── My Orders Tab ── */}
      {activeTab === 'my-orders' && (isPetOwner || isVet) && (
        <section className="mt-8 rounded-3xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 p-6 shadow-sm sm:p-8">
          <h2 className="font-display text-xl font-semibold text-dark dark:text-white sm:text-2xl mb-2">My Orders</h2>
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">Track your purchases and view order history.</p>

          {myOrdersLoading && <p className="text-sm text-slate-500 animate-pulse">Loading orders...</p>}
          ...
          {!myOrdersLoading && myOrders.length === 0 && (
            <div className="text-center py-12">
              ...
              <Link href="/shop" className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white hover:bg-primary/90 transition">
          ...
          <div className="space-y-4">
            {myOrders.map((order) => (
              <div key={order.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4 gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="font-semibold text-slate-900 dark:text-white">Order #{order.id.split('-')[0]}</h3>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                        order.status === 'paid' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-300' :
                        order.status === 'created' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' :
                        'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'
                      }`}>
                        {order.status.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Placed on {new Date(order.created_at).toLocaleDateString()}</p>
                  </div>
                  <div className="text-left sm:text-right">
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-0.5">Total Amount</p>
                    <p className="font-bold text-slate-900 dark:text-white"><IndianRupee className="inline w-3.5 h-3.5 -mt-0.5 mr-0.5" />{order.amount.toFixed(2)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {order.items && order.items.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-4">
                      <div className="h-12 w-12 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 overflow-hidden flex-shrink-0">
                        {item.image_url ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">Qty: {item.quantity} × <IndianRupee className="inline w-3 h-3 -mt-0.5" />{Number(item.price).toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                </div>
