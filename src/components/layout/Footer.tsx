'use client';

import Link from 'next/link';
import { PawPrint, Facebook, Instagram, Twitter } from 'lucide-react';

export const Footer = () => {
  return (
    <footer
      className="mt-16 transition-colors duration-300"
      style={{
        borderTop: '1px solid var(--border-color)',
        backgroundColor: 'var(--bg-secondary)',
      }}
    >
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid gap-10 md:grid-cols-[1.5fr,1fr,1fr,1.5fr]">
          <div>
            <div className="mb-4 flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary text-white">
                <PawPrint className="h-4 w-4" aria-hidden="true" />
              </div>
              <span className="font-display text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Aniwoo</span>
            </div>
            <p className="max-w-xs text-sm" style={{ color: 'var(--text-secondary)' }}>
              Complete pet care solutions from nutrition to grooming, vet services to AI health checks.
            </p>
          </div>

          <div>
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Quick Links</h3>
            <ul className="space-y-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
              <li>
                <Link href="/" className="transition hover:text-primary">
                  Home
                </Link>
              </li>
              <li>
                <Link href="/shop" className="transition hover:text-primary">
                  Services &amp; Shop
                </Link>
              </li>
              <li>
                <Link href="/ai-health-check" className="transition hover:text-primary">
                  AI Health Check
                </Link>
              </li>
              <li>
                <Link href="/about" className="transition hover:text-primary">
                  About
                </Link>
              </li>
              <li>
                <Link href="/contact" className="transition hover:text-primary">
                  Contact
                </Link>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Contact</h3>
            <ul className="space-y-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
              <li>Email: support@aniwoo.pet</li>
              <li>Phone: +1 (555) 987-1234</li>
              <li>Address: 123 Pet Lane, Paw City</li>
            </ul>
            <div className="mt-3 flex gap-3" aria-label="Social media links">
              <button
                type="button"
                className="flex h-8 w-8 items-center justify-center rounded-full transition hover:border-primary hover:text-primary"
                style={{ border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}
                aria-label="Aniwoo on Facebook"
              >
                <Facebook className="h-4 w-4" />
              </button>
              <button
                type="button"
                className="flex h-8 w-8 items-center justify-center rounded-full transition hover:border-primary hover:text-primary"
                style={{ border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}
                aria-label="Aniwoo on Instagram"
              >
                <Instagram className="h-4 w-4" />
              </button>
              <button
                type="button"
                className="flex h-8 w-8 items-center justify-center rounded-full transition hover:border-primary hover:text-primary"
                style={{ border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}
                aria-label="Aniwoo on X (Twitter)"
              >
                <Twitter className="h-4 w-4" />
              </button>
            </div>
          </div>

          <div>
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Newsletter</h3>
            <p className="mb-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
              Get pet care tips, product recommendations, and exclusive offers delivered to your inbox.
            </p>
            <form
              className="flex flex-col gap-2 sm:flex-row"
              onSubmit={(e) => {
                e.preventDefault();
                // TODO: Integrate with newsletter backend
              }}
            >
              <label className="sr-only" htmlFor="newsletter-email">
                Email address
              </label>
              <input
                id="newsletter-email"
                type="email"
                required
                placeholder="Enter your email"
                className="w-full rounded-full px-4 py-2 text-sm outline-none ring-primary/20 focus:ring-2 transition-colors duration-200"
                style={{
                  backgroundColor: 'var(--bg-tertiary)',
                  border: '1px solid var(--border-color)',
                  color: 'var(--text-primary)',
                }}
              />
              <button
                type="submit"
                className="whitespace-nowrap rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-primary/90"
              >
                Subscribe
              </button>
            </form>
          </div>
        </div>

        <div
          className="mt-8 flex flex-col items-center justify-between gap-4 pt-4 text-xs sm:flex-row"
          style={{ borderTop: '1px solid var(--border-color)', color: 'var(--text-muted)' }}
        >
          <p>&copy; 2024 Aniwoo. All rights reserved.</p>
          <div className="flex gap-4">
            <button type="button" className="transition hover:text-primary">
              Privacy Policy
            </button>
            <button type="button" className="transition hover:text-primary">
              Terms
            </button>
          </div>
        </div>
      </div>
    </footer>
  );
};
