  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-200 transition hover:bg-slate-50 dark:bg-slate-800 hover:border-slate-400 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white dark:bg-slate-900 disabled:hover:border-slate-300 dark:border-slate-700"
      ...
      {showRoleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md space-y-6 rounded-3xl bg-white dark:bg-slate-900 p-8 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-2xl font-semibold text-dark dark:text-white">Select Account Type</h3>
              <button
                onClick={() => setShowRoleModal(false)}
                className="rounded-full p-2 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 hover:text-slate-600 dark:text-slate-300 transition"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="grid gap-3">
              <button
                ...
                className="group flex w-full items-center justify-start rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 text-left transition hover:border-primary hover:bg-primary/5"
              >
                ...
                <div>
                  <div className="font-semibold text-dark dark:text-white group-hover:text-primary transition-colors">Veterinarian</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">Manage your clinic, accept appointments</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="font-semibold text-dark dark:text-white transition-colors">Pet Owner</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">Book vets, manage pet health</div>
                </div>
              </button>
            </div>
          </div>
